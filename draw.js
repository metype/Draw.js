...
const error = consoconst print = console.log;
const println = console.log;
...
var rectangleModeAPI = CORNER;
var ellipseModeAPI = CORNER;
var keyCode = null;
var mouseX = 0;
var mouseY = 0;
var pmouseX;
...
function rect(x = 0, y = 0, w = 1, h = 1) {
  ...
  canvas.stroke();
	}else if(rectangleModeAPI===CORNER){
		 if (fillable) { canvas.fillRect(x + translationX, y + translationY, w, h); } else { canvas.rect(x  + translationX, y  + translationY, w, h); }
  canvas.stroke();
	}
}
function ellipse(x = 0, y = 0, r = 1) {
  var c = document.getElementById("APICanvas");
  var canvas = c.getContext("2d");
	if(ellipseModeAPI===CENTER){
  canvas.beginPath();
  canvas.arc(x - (r / 2) + translationX, y - (r / 2) + translationY, r, 0, 2 * Math.PI)
  canvas.stroke();
	} else if(ellipseModeAPI===CORNER){
		  canvas.beginPath();
  canvas.arc(x + translationX, y + translationY, r, 0, 2 * Math.PI)
  canvas.stroke();
	}
}
